...
if not (CLASSIC_API_VERSION and SUPERWOW_VERSION) then
	if DEFAULT_CHAT_FRAME then
		DEFAULT_CHAT_FRAME:AddMessage("|cffff2020[MSBT Fatal Error]|r MikScrollingBattleText requires ClassicAPI.dll & SuperWoW! Please ensure ClassicAPI.dll and SuperWoW are loaded.", 1, 0.2, 0.2)
	end
	return
end
...
do
 ...
 for dirType, prefix in pairs(DIRECTION_PREFIX) do
  ...
  -- Spell-only actions fallback to SPELL_ variant
  dmgEventStr[dirType]["DOT"] = prefix .. "SPELL_DOT";
  dmgEventStr[dirType]["RESIST"] = prefix .. "SPELL_RESIST";
  dmgEventStr[dirType]["REFLECT"] = prefix .. "SPELL_REFLECT";
 ...
 -- Specific pet event overrides
 dmgEventStrSpell[3]["DOT"] = "MSBT_EVENTTYPE_OUTGOING_SPELL_DOT";
 dmgEventStr[3]["DOT"] = "MSBT_EVENTTYPE_OUTGOING_SPELL_DOT";
 dmgEventStrSpell[3]["REFLECT"] = "MSBT_EVENTTYPE_OUTGOING_SPELL_REFLECT";
 dmgEventStr[3]["REFLECT"] = "MSBT_EVENTTYPE_OUTGOING_SPELL_REFLECT";
...
function MikSBT.CombatEventsHandler(combatEvent)
 ...
 if (MikSBT.CurrentProfile.EventSettings[eventTypeString]) then
  -- Suppress Vigor energy gains if disabled by user.
  if (combatEvent.NotificationType == MikCEH.NOTIFICATIONTYPE_POWER_GAIN and MikSBT.CurrentProfile.ShowVigorGains == false) then
   local isVigor = false;
   if (combatEvent.SpellId and (combatEvent.SpellId == 14983 or combatEvent.SpellId == 14984)) then
    isVigor = true;
   elseif (combatEvent.EffectName and type(combatEvent.EffectName) == "string") then
    local vigorName = MikSBT.SpellName(14983, "Vigor");
    if (combatEvent.EffectName == vigorName or string.lower(combatEvent.EffectName) == "vigor" or string.find(string.lower(combatEvent.EffectName), "vigor") ~= nil) then
     isVigor = true;
    end
   end
   if (isVigor) then
  ...
  if type(profile) == "table" then
   if not profile.CreationVersion then
    ...
    if MikSBT.DEFAULT_CONFIG then
     if not profile.EventSettings then profile.EventSettings = {} end
     for evtName, defaultSetting in pairs(MikSBT.DEFAULT_CONFIG.EventSettings or {}) do
      if not profile.EventSettings[evtName] then
       profile.EventSettings[evtName] = MikSBT.CopyTable(defaultSetting)
      end
     end
     if not profile.Triggers then
      profile.Triggers = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.Triggers or {})
     end
     if not profile.Suppressions then
      profile.Suppressions = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.Suppressions or {})
     end
     if not profile.BlizzardFontSettings then
      profile.BlizzardFontSettings = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.BlizzardFontSettings or {})
     end
    end

    if profile.ShowVigorGains == nil then profile.ShowVigorGains = true end
    if profile.LowHealthSound == nil then profile.LowHealthSound = true end
    if profile.LowManaSound == nil then profile.LowManaSound = true end
    profile.CreationVersion = 6.1
   end
  end
 end
end
...
function MikSBT.SetOptionFromVarPath(varPath, newValue)
 local currentTable = MikSBT.CurrentProfile;
 if (not currentTable or not varPath) then return end
 ...
 currentTable[keys[numKeys]] = newValue;
end
...
function MikSBT.AddAnimation(animationEvent)
 ...
 local texture = animationEvent.Texture;

	-- SpellId-based icon lookup (fast DBC path via SuperWoW or NamPower)
	...
	if not texture and animationEvent.EffectName then
		local name = string_gsub(animationEvent.EffectName, " %(%d+%)", "")
		local lname = strlower(name)
		local cached = ICON_CACHE[lname]
		if cached then
			texture = cached
		elseif not NO_ICON_CACHE[lname] then
			if MikCEH.hasNampower and GetSpellIdForName then
				local ok, sid = pcall(GetSpellIdForName, name)
				if ok and sid and sid ~= 0 then
					if MikCEH.hasSuperWoW and SpellInfo then
						local ok2, _, _, tex = pcall(SpellInfo, sid)
						if ok2 and tex then texture = tex; ICON_CACHE[lname] = tex end
					end
					if not texture and GetSpellRecField and GetSpellIconTexture then
						...
						if ok2 and iconId then
							...
							if ok3 and tex then texture = tex; ICON_CACHE[lname] = tex end
						end
					end
				end
			end
			if not texture and GetItemInfo then
				local ok, _, _, _, _, _, _, _, _, tex = pcall(GetItemInfo, name)
				if ok and tex then
					texture = tex
					ICON_CACHE[lname] = tex
				end
			end
			if not texture then
				NO_ICON_CACHE[lname] = 1
			end
		end
	end
...
function MikSBT.RepositionAnimDisplayInfo(scrollArea)
 ...
 local numNonStickies = #activeNonStickies;
...
function MikSBT.RepositionStickyAnimDisplayInfo(scrollArea)
 ...
 local numStickies = #activeStickies;
...
function MikSBT.ScrollUp(animDisplayInfo, scrollArea)
 ...
 local dtScale = (animDisplayInfo.DeltaTime and animDisplayInfo.DeltaTime > 0) and (animDisplayInfo.DeltaTime / 0.02) or 1;
 animDisplayInfo.PositionY = animDisplayInfo.PositionY + (MikSBT.CurrentProfile.AnimationStep * dtScale);
...
function MikSBT.CommandHandler(params)
 ...
 if (currentParam == "") then
  if (MSBTFrameOptions) then
   if (MSBTFrameOptions:IsShown()) then
    MSBTFrameOptions:Hide();
   else
    if (MSBTScrollAreaMoverControlFrame and not MSBTScrollAreaMoverControlFrame:IsShown()) or (not MSBTScrollAreaMoverControlFrame) then
 ...
 elseif (currentParam == MikSBT.COMMAND_VIGOR) then
  if (MikSBT.CurrentProfile.ShowVigorGains == false) then
   MikSBT.CurrentProfile.ShowVigorGains = true;
  else
   MikSBT.CurrentProfile.ShowVigorGains = false;
  end
  local status = MikSBT.CurrentProfile.ShowVigorGains and "enabled" or "disabled";
  MikSBT.Print("Vigor energy gains are now " .. status .. ".", 1, 1, 1);
