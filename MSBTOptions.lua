...
local selectedClassFilter = "Shaman";
...
function MikSBTOpt.OnEvent()
 if (event == "VARIABLES_LOADED") then
  ...
  if (MikSBT and MikSBT.CurrentProfile and not MikSBTOpt.isInitialized) then
   MikSBTOpt.Init();
   MikSBTOpt.isInitialized = true;
  end
 end
end
...
function MikSBTOpt.Init()
 -- Ensure MikSBT profile is loaded
 if (not MikSBT or not MikSBT.CurrentProfile or not MikSBT_Save) then
  return;
 end
...
function MikSBTOpt.OnShow()
 -- Ensure options are initialized on show
 if (not MikSBTOpt.isInitialized) then
  MikSBTOpt.Init();
  MikSBTOpt.isInitialized = true;
 else
  MikSBTOpt.PopulateProfileOptions();
...
function MikSBTOpt.CountTriggers()
 local numTriggers = 0;
 if (MikSBT and MikSBT.CurrentProfile and MikSBT.CurrentProfile.Triggers) then
...
function MikSBTOpt.CountSuppressions()
 ...
 if (MikSBT and MikSBT.CurrentProfile and MikSBT.CurrentProfile.Suppressions) then
